...
class Yar
{
    ...
    public static function call($mapName, $parameters)
    {
        ...
        if (isset($mapConfig['connect_timeout'])) {
            $yarClient->setOpt(YAR_OPT_CONNECT_TIMEOUT, $mapConfig['connect_timeout']);
        }

        if (isset($mapConfig['read_timeout'])) {
            $yarClient->setOpt(YAR_OPT_TIMEOUT, $mapConfig['connect_timeout']);
        }
